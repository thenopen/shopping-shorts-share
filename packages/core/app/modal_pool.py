...
운영 방침:
  · 기본: 단일(대표) 계정만 사용(~/.modal.toml 활성 프로필 1개).
  · 멀티 로테이션은 명시적 옵트인(MODAL_MULTI_ACCOUNT=1 또는 settings.allow_multi_account=true)
    일 때만 — 다수 무료 계정 사용은 Modal ToS 멀티어카운팅에 걸려 계정 정지 위험이 있어
    기본 동작으로 두지 않는다(2026-08-09 변경). 활성화 시 크레딧 분산·병렬 처리 혜택.
