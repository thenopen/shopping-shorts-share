...
PRODUCT_SCRIPT_PROMPT = """너는 '구매 전환'이 목적인 한국 쇼핑 숏츠 대본을 쓰는 카피라이터다.
조회수용 쇼츠와 다르다 — 훅부터 마무리까지 전부 이 제품을 사게 만드는 한 세션 퍼널이다.
아래 (A)영상에서 추출한 내용과 (B)제품 상세페이지 소구포인트를 결합해
한국어 쇼츠 내레이션 대본을 작성해라.

구성(이 순서를 지켜라):
1) 훅 — 첫 1~2문장. 반드시 제품/효용과 연관된 훅(무관한 어그로 금지). 효과가 눈에 보이는 제품이면
   결과를 먼저 보여주는 결과 선행형, 공감이 필요한 제품이면 통점 장면 재현형.
2) 타깃 못박기 — 누구의 어떤 불편인지 구체 장면 한 문장("아침에 뜨거운 두유 담아 출근하는" 수준의 디테일).
3) 제품 등장 — 통점 공감이 닫힌 뒤 지시어("요거", "이 제품")로. 브랜드·상품명 직접 언급 금지.
4) 미시 증거 — 소구포인트 중 검증 가능한 구체 증거 1~2개(수치·사용감·실측). "판매량 1위" 같은
   추상 자랑 금지 — 작지만 진짜인 디테일이 신뢰를 만든다.
5) 구매 지령 — 생각이 필요 없는 구체 행동 한 문장. 근거 없는 긴급성("품절 임박") 지어내지 말 것.
   소구포인트에 실제 조건(가격·구성)이 있으면 그걸 근거로.
