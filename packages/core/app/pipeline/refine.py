...
- 이모지·특수기호(✨🤦 등) 금지 — TTS가 못 읽고 자막을 오염시킨다. 감탄은 말로 써라."""

# 이모지·픽토그램 제거용 — TTS 낭독 불가 + 자막 오염(실측: 생성 대본에 ✨🤦‍♀️ 출현).
...
def _normalize_script(text: str) -> str:
    """대본 형식 정규화 — 경로(생성/가공/받아쓰기)마다 널뛰는 공백·빈 줄·이모지를 계약 형식으로 수렴."""
    lines = []
    for ln in (text or "").splitlines():
        s = _EMOJI_RE.sub("", ln)
        s = _re.sub(r"[ \t]+", " ", s).strip()   # 연속 공백·탭 → 스페이스 1개
        if s:
            lines.append(s)
    return "\n".join(lines)
...
def _length_hint(target_seconds: float | None) -> str:
    ...
    sents = max(4, int(round(chars / 11)))
    return (f"**글자수가 1순위 제약이다**: 공백 제외 {lo}~{hi}자(목표 {sec}초). "
            f"줄 수는 참고용으로 약 {sents}줄 안팎이 된다. "
            f"⚠ 가장 흔한 실패는 분량 미달 — 줄 수를 채웠어도 {lo}자 미만이면 실패다. "
            f"구성 5단은 '순서'일 뿐, 각 단계를 여러 줄로 확장하고 특히 4) 미시 증거를 "
            f"소구포인트별로 하나씩 모두 다뤄 분량을 채워라.")
